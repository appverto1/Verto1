            ...
            <div className="w-full max-w-md relative z-10 flex flex-col h-full">
                <div className="flex justify-between items-center mb-6 sm:mb-8">
                    <div className="flex items-center gap-3">
                         ...
                         <div>
                             <h1 className="text-xl sm:text-2xl font-bold text-[#4318FF]">Oi, {user.name}!</h1>
                             <p className="text-[10px] sm:text-sm text-gray-500 font-bold">Vamos ver seus poderes?</p>
                         </div>
                    </div>
                ...
                <div className="bg-white p-4 sm:p-6 rounded-[32px] sm:rounded-[40px] shadow-xl border border-blue-50 flex flex-col justify-center mb-6 relative overflow-hidden">
                    ...
                    <div className="flex items-center justify-between mb-4 sm:mb-6 px-1 sm:px-2">
                        <h2 className="text-lg sm:text-xl font-bold text-gray-700 flex items-center gap-2"><Trophy className="text-yellow-500" size={20}/> Nível de Herói</h2>
                        <div className="relative w-28 sm:w-32">
                            <select 
                                className="w-full bg-blue-50 border border-blue-100 text-[9px] sm:text-[10px] font-bold text-[#4318FF] px-2 sm:px-3 py-1.5 sm:py-2 rounded-xl outline-none appearance-none uppercase tracking-wider"
                ...
                <div className="mt-4 mb-10"> 
                  <div className="flex items-center justify-between mb-4">
                     <h2 className="text-sm font-bold text-gray-400 uppercase tracking-wider flex items-center gap-2 tracking-widest"> <Trophy size={16} /> Histórico </h2>
                  </div>
                  <div className="border-l-2 border-gray-200 ml-3 pb-4 pl-6 space-y-8"> 
                    {sortedDateKeys.length > 0 ? sortedDateKeys.map(dateKey => ( 
                      <div key={dateKey} className="relative animate-slide-up"> 
                        ...
                        <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-4 bg-gray-100 px-3 py-1 rounded-full w-fit tracking-widest">{dateKey}</h3> 
    ...
    <div className="min-h-screen bg-[#F4F7FE] p-6 flex flex-col overflow-x-hidden"> 
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6"> 
        <div className="flex items-center gap-4"> 
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800 tracking-tight leading-none"> Olá, <span className="text-[#4318FF]">{user.name}</span>! </h1> 
            <p className="text-gray-500 font-medium mt-1 text-xs"> Vamos cuidar de você hoje? </p> 
          </div>
        </div> 
        <div className="flex gap-2 w-full sm:w-auto"> 
          ...
          <button onClick={onLogout} className="flex-1 sm:flex-none p-3 bg-white rounded-2xl shadow-sm text-gray-400 hover:text-red-500 hover:bg-red-50 transition-all flex items-center justify-center" > <LogOut size={20}/> </button> 
        </div> 
      </div> 
      {!dailyCheckinDone ? ( 
        <div className="mb-8 animate-slide-up"> 
          ...
          <p className="text-center text-xs text-gray-400 font-medium animate-pulse mt-[-10px] italic"> Deslize para escolher ou clique </p> 
        </div> 
      ) : ( 
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar"> 
        <div className="mb-8"> 
          <h2 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4 flex items-center gap-2 tracking-widest"> <Zap size={16} /> Suas Atividades </h2> 
              ...
              {tasks.map((task: any) => ( 
                <div key={task.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between group hover:shadow-md transition-all"> 
                  <div className="flex items-center gap-4"> 
                    ...
                    <div> <h3 className="font-bold text-gray-800 tracking-tight">{task.title}</h3> {task.description && <p className="text-xs text-gray-500 line-clamp-1">{task.description}</p>} </div> 
                  </div> 
        ...
        {dailyReviewTasks.length > 0 && (
          <div className="mb-8 animate-fade-in">
            <h2 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4 flex items-center gap-2 tracking-widest"> <BookOpen size={16} /> Revisão Diária </h2>
            <div className="grid gap-3">
              {dailyReviewTasks.map((review: any) => (
                <div key={review.id} className="bg-green-50/50 p-5 rounded-[32px] border border-green-100 shadow-sm">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      <h3 className="font-bold text-gray-800 tracking-tight">{review.title}</h3>
                      <p className="text-[10px] text-green-600 font-bold uppercase">Lembrete de Revisão</p>
                    </div>
                  </div>
                  <div className="bg-white/80 p-4 rounded-2xl border border-green-50 mb-3">
                    <p className="text-xs text-gray-400 font-bold uppercase mb-1">Sua Resposta:</p>
                    <p className="text-sm text-gray-700 leading-relaxed italic">"{review.note}"</p>
                  </div>
        ...
        <div className="mt-8 mb-6"> 
          <div className="flex items-center justify-between mb-4">
             ...
             <div className="relative w-48">
                <select 
                  className="w-full bg-white border border-gray-200 text-xs font-bold text-gray-600 px-3 py-2 rounded-xl outline-none focus:border-[#4318FF] transition-all appearance-none"
                  value={selectedSnapshotDate}
                  onChange={(e) => setSelectedSnapshotDate(e.target.value)}
                >
      ...
      {isMoodModalOpen && selectedMood && ( 
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fade-in"> 
          <div className="bg-white w-full max-w-sm rounded-[32px] p-6 shadow-2xl animate-pop relative"> 
            ...
            <div className="grid grid-cols-2 gap-3 mb-4"> 
              {energyTags.map((tag: any) => ( 
                <button key={tag.id} onClick={() => setSelectedTag(selectedTag === tag.id ? null : tag.id)} className={`p-3 rounded-2xl flex items-center gap-3 border-2 transition-all ${selectedTag === tag.id ? 'border-[#4318FF] bg-[#4318FF]/5 shadow-sm' : 'border-transparent bg-gray-100'}`} > 
                  <div className={`p-2 rounded-xl ${tag.color} shrink-0`}> {tag.icon} </div> <span className={`text-xs font-bold ${selectedTag === tag.id ? 'text-[#4318FF]' : 'text-gray-500'}`}>{tag.label}</span> 
                </button> 
              ))} 
            </div> 
            ...
            <button onClick={handleConfirmMood} disabled={showMoodSuccess} className={`w-full font-bold text-lg py-3 rounded-2xl shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2 ${showMoodSuccess ? 'bg-green-500 text-white' : 'bg-[#4318FF] text-white shadow-[#4318FF]/30 hover:opacity-90'}`} > {showMoodSuccess ? <React.Fragment><CheckCircle size={24}/> Sucesso!</React.Fragment> : <React.Fragment><Check size={20} strokeWidth={3} /> Registrar</React.Fragment>} </button> 
          </div> 
        </div> 
      )} 
      {selectedTask && ( 
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fade-in"> 
          <div className="bg-white w-full max-w-sm rounded-[32px] p-6 shadow-2xl animate-pop relative"> 
            ...
            <div className="text-center mb-6"> 
              ...
              <h3 className="text-xl font-bold text-gray-800 tracking-tight">{selectedTask.title}</h3> 
            ...
            {selectedTask.type === 'intervention' ? (
              <div className="animate-slide-up space-y-4">
                <div className="bg-green-50 p-4 rounded-2xl border border-green-100">
                  <p className="text-sm text-gray-700 font-medium leading-relaxed italic">"{selectedTask.description}"</p>
                </div>
                <textarea 
                ...
                <button 
                  ...
                  className={`w-full font-bold text-lg py-3 rounded-2xl shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2 ${showTaskSuccess ? 'bg-green-500 text-white' : 'bg-green-600 text-white shadow-green-600/30 hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed'}`}
                > 
              ...
              <div className="animate-slide-up"> 
                {user.age <= 12 ? (
                  <div className="space-y-6">
                    <p className="text-center text-sm font-bold text-gray-600 mb-4">Como você se saiu nessa missão?</p>
                      ...
                      {selectedTask.maxScore === 1 ? (
                          ...
                          {[0, 0.5, 1].map((score) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className="text-[8px] font-bold uppercase">{labels[score]}</span>
                              </button>
                            );
                          })}
                        </>
                        ...
                        Array.from({ length: (selectedTask.maxScore || 2) + 1 }, (_, i) => i).map((score) => {
                          ...
                          return (
                            ...
                            >
                              <span className="text-2xl font-bold">{score}</span>
                              <span className="text-[8px] font-bold uppercase">{labels[score] || "Pontuação"}</span>
                            </button>
                          );
                        })
                      )}
                  ...
                  <React.Fragment>
                    ...
                    <p className="text-center text-xs text-[#4318FF]/50 mt-4 font-medium animate-pulse italic">(Solte para continuar)</p> 
                  </React.Fragment>
                )}
              </div> 
            ) : ( 
              <div className="animate-slide-up"> 
                <div className="mb-4"> 
                  <div className="flex items-center justify-between mb-2"> 
                    <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">{user.age <= 12 ? 'Sua Pontuação' : 'Energia'}</span> 
                    <span className="text-sm font-bold text-[#4318FF]">{user.age <= 12 ? taskEnergy : `${taskEnergy}%`}</span> 
                  </div> 
                ...
                <button onClick={handleConfirmTask} disabled={showTaskSuccess} className="w-full font-bold text-lg py-3 rounded-2xl shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2 bg-green-500 text-white shadow-green-500/30 hover:opacity-90" > {showTaskSuccess ? <React.Fragment><CheckCircle size={24}/> Sucesso!</React.Fragment> : <React.Fragment><Check size={20} strokeWidth={3} /> Concluir</React.Fragment>} </button> 
              </div> 
            )} 
          </div> 
        </div> 
